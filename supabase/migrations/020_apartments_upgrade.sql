-- ALYA APART - Upgrade existing apartments schema (Phase 20)
-- Safe to re-run. Does NOT drop or recreate tables. Does NOT delete data.
-- Use when apartments table exists but is missing columns from 018_apartments.sql.

ALTER TABLE public.apartments
  ADD COLUMN IF NOT EXISTS name TEXT NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS description TEXT NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS cover_photo_path TEXT,
  ADD COLUMN IF NOT EXISTS feature_near_sea BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS feature_wifi BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS feature_ac BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS feature_kitchen BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS feature_balcony BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS feature_family_friendly BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

-- apartment_photos: create only if missing, then add any missing columns
